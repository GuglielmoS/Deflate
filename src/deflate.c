    ...
    while (!finished) {
        ...
        if (in_fs->is_finished || i < 3) {
            ...
            if (chain == NULL) {
                ...
                while (chain != NULL) {
                    ...
                    if (k == 3) {
                        while (in_fs->n_available_bytes > 2 && k < LZ_MAX_SEQ_LEN && 
                               history_buf->next_pos < HISTORY_BUFFER_SIZE) {
                        ...
                        if (in_fs->n_available_bytes <= 2) {
                            uint8_t hist_buf_byte = History_Buffer_get(history_buf,buf_start_pos+k);
                            if (in_fs->buffer[in_fs->buf_pos] == hist_buf_byte) k++;
                        }
                ...
                if (max_seq_pos == -1 || max_seq_length < LZ_MIN_SEQ_LEN) {
                    LZ_Queue_enqueue(&lz_queue, LZ_Literal_new(next3B[0]));
                    lit_count++;
                    ...
                    LZ_Queue_enqueue(&lz_queue,LZ_Pair_new((LZ_Pair){max_seq_pos,max_seq_length}));
                    pair_count++;
