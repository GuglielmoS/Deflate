...
{
    return LZ_Element_get_pair(e).distance;
}
...
{
    return LZ_Element_get_pair(e).length;
} 
...
{
    LZ_Element_set_type(e, _LZ_PAIR);
}
...
{
	LZ_Element_set_type(e, _LZ_LITERAL);
}
...
{
    if (LZ_Element_is_literal(src)) {
    	LZ_Element_set_literal(dest, LZ_Element_get_literal(src));
    }
    else {
        LZ_Element_set_pair(dest, LZ_Element_get_pair(src));
    }
}
...
{
    return !LZ_Element_is_literal(e);
}
